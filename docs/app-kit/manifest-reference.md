...
declares an out-of-process consumer — a `backend.entryPoint`, or a stdio
`mcpServers` entry (one without `url`) — and declares no `backend.hooks`. Those
dependencies are not the Gateway's to import: the entry point runs as its own
process, a stdio server is spawned as its own process, and the runtime
...
would ever provision the file), and to an app that ALSO declares a hook — a hook
is imported into the Gateway process, which the deps tree never reaches, so its
imports would be broken by an install that reported success. "Declares a hook"
is read from the same normalized manifest the Gateway's own loaders are handed,
so a `hooks` value the manifest parser drops is not one. On a source install
nothing changes — `requirements.txt` is
still pip-installed at install time — so an app that needs its dependencies
importable by hooks must ship `pyproject.toml`/`setup.py` and accept that it
cannot install on the desktop build.
