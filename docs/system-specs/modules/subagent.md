...
- `_force_reap`: take the process handle → reset with 30s timeout → SIGKILL fallback on a hang, a raised reset or a survivor → mark done → audit on the kill's result → fire `subagent_done` WS event. **The force-stop path records what actually happened to the process, in the cron reaper's vocabulary** (the two paths must not diverge): `_sigkill_session(session_key, handle)` returns `None` once the process group has been signalled or there was nothing to kill, and otherwise the failure it hit — the broadcast guard's refusal of the pid (`ValueError: kill_process_tree: refusing …`), a group kill that raised with no pid-scoped fallback landing (`PermissionError`; on Windows any error other than a gone tree, since the fallback is a root-only `taskkill` and nothing sweeps the descendants there), a live pid whose identity cannot be confirmed as the run's (no start id recorded, or none readable now), or an error in the kill path ahead of the signal — and never raises, because its caller owns a teardown it must still finish. `_force_reap` folds that into the run's error text (`…; kill failed: <reason>`, appended after its own synthesis and before the tombstone; a user stop keeps `outcome == "stopped"` and gains the text) and audits `reaper_force_kill` as `failed` rather than `reaped`; `_teardown_run_session` (the run's own `finally`) audits `run_finally_force_kill` as `failed` rather than `sigkill` with the reason in the row's `error`, because the run's own record is already written by then. Nothing to kill is not a failure — no pre-reset handle, no usable pid, a pid already exited or recycled at either read, a group and pid both gone when the signal is sent — and keeps `reaped` / `sigkill`. **The kill handle is taken BEFORE the reset, retained under the run's id, and is the only thing that names the process**: `SessionLifecycle.reset` pops the session from the map under its lock before the awaits that can hang, and the common shape is the run's own `finally` being the reset that hangs while the reaper (a deadline, a user Stop) arrives to act on it — so a handle the reaper took before ITS reset would see an empty map. Both callers therefore go through `_retain_process_handle(agent_id, session_key)` immediately ahead of their reset: a session still live under the key is read (pid, recorded start id, child records — `_ProcessHandle`) and RETAINED in `_process_handles` under the run's id; on a map miss the entry the other path retained is returned instead. The map is read once, there, and never after a reset — a session under the key afterwards is a successor a cold start registered during the reset's awaits, and killing it would leave the run's own process alive. The entry is cleared once the path holding it has decided (the handle was consumed by the kill, or the survivor check found the process gone); the run's `finally` clears in a `finally` of its own, so a teardown the reaper cancels mid-reset leaves nothing behind. **The root is validated by both readings, and a completed reset is not proof of death**: `_sigkill_session` asks `platform_compat.pid_exists` first (on Windows the exit-code-confirmed `OpenProcess` + `GetExitCodeProcess` probe) and then re-reads the start id (`platform_compat.get_process_start_id`, the recycling detector) against the recorded one — not through the child verifier, which denies a root that has no recorded basename — both before anything reads through the pid and again immediately before the signal, because the child walk awaits and the root can exit (its pid reused) meanwhile; a missing process or a changed reading is a gone root (nothing to kill; only the children recorded before the reset are swept). A reset that returned — `True`, or `False` for a key the other path had already popped — is not proof the process is gone (its own shutdown can fail without raising out of it; a `False` one stopped nothing), so both callers ask the handle afterwards (`_process_survived`: the process must exist AND its start id must match; either failing is gone — no usable pid, no process behind it, a recycled pid — while an existing pid whose identity cannot be read still stands and the kill then names it) and run the fallback for a survivor, whose outcome is what the record says. Existence is asked first because an identity-only comparison bypasses the exit-code-confirmed path: a start id reads back for an EXITED Windows process as long as any handle to it is open (asyncio's Proactor transport keeps one until GC), which made the fallback signal a dead pid and record the error as a failed kill.
...
- `_sigkill_session(session_key, handle)`: best-effort SIGKILL when graceful reset hangs or did not end the process; acts on the pre-reset handle only, returns the failure it hit instead of swallowing it (contract in the `_force_reap` bullet above)
