...
def _refuse_multilinked_credential_leaves() -> tuple[str, ...]:
    ...
    * the other name is FOUND under a data home -- returned, for the caller to add to this
      ...
      the tension the other two trade against.
    * the other name is NOT under any data home -- the snapshot and backup case (``cp -al``,
      rsnapshot, ``rsync --link-dest``), which cannot be masked because it cannot be located
      without walking the filesystem. REFUSED in the live home, which is the cost this
      control has always accepted there, and reported elsewhere.
    * the leaf is absent, is a link, or is not a regular file -- skipped. Only a regular file
    ...
    for root in _masked_crew_home_roots():
        for leaf in sorted(_CREW_HARDLINK_REFUSED_LEAVES):
            ...
            if aliases:
                # The other name is known, so the hole can be CLOSED instead of traded
                # against. Hiding it makes the bytes unreachable in every namespace, which
                ...
                to_hide.extend(aliases)
                continue
            # No other name under any home this pass can read, so the second one is outside
            # them -- the snapshot and backup case (``cp -al``, rsnapshot,
...
def masked_credential_leaf_aliases() -> list[tuple[str, int, str]]:
    ...
    Returns the leaf path, its link count, and the home it was found under. The caller
    renders the same sentence the spawn would use instead of paraphrasing it, and needs the
    home to say whether that spawn REFUSES or only reports: only the live home refuses, so a
    reader told otherwise would act on a failure that is not coming.
    """
    ...
    for root in _masked_crew_home_roots():
        for leaf in sorted(_CREW_HARDLINK_REFUSED_LEAVES):
            ...
            try:
                ...
                continue
            if stat.S_ISREG(info.st_mode) and info.st_nlink > 1:
                found.append((target, info.st_nlink, root))
    return found
